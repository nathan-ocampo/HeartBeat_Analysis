...
#%%
if os.path.exists('{c}_{t}{h}_t{tp}_analysis.xlsx'.format(c = con, h = hrt, t = treatment, tp = tps[0])):
...
freqOt.add_series({
    'categories': ['Summary', 1, 1, len(tps), 1],
    'values':     ['Summary', 1, 2, len(tps), 2],
})
...
freqDis.add_series({
    'name':       'tp0',
    'categories': ['Freq_Distribution', 1, 1, 1, len(f)],
    'values':     ['Freq_Distribution', 2, 1, 2, len(P1_tp0)],
})
...
freqDis.add_series({
    'name':       'tp1',
    'categories': ['Freq_Distribution', 1, 1, 1, len(f)],
    'values':     ['Freq_Distribution', 3, 1, 3, len(P1_tp1)],
})
...
freqDis.add_series({
    'name':       'tp2',
    'categories': ['Freq_Distribution', 1, 1, 1, len(f)],
    'values':     ['Freq_Distribution', 4, 1, 4, len(P1_tp2)],
})
...
#Add frequency distribution for tp3
    freqDis.add_series({
        'name':       'tp3',
        'categories': ['Freq_Distribution', 1, 1, 1, len(f)],
        'values':     ['Freq_Distribution', 5, 1, 5, len(P1_tp3)],
    })
...
#Add frequency distribution for tp3
    freqDis.add_series({
        'name':       'tp4',
        'categories': ['Freq_Distribution', 1, 1, 1, len(f)],
        'values':     ['Freq_Distribution', 6, 1, 6, len(P1_tp3)],
    })
...
strainOt.set_x_axis({'name': 'Time (min)', 'min': 0, 'max': tps[-1]+160})
...
freqOt.set_x_axis({'name': 'Time (min)', 'min': 0, 'max': tps[-1]+160})
...
summary.insert_chart('F1',freqOt)
summary.insert_chart('F17',strainOt)
